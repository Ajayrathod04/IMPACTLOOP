...
class Change(BaseModel):
    ...
    unresolved_risks: List[str] = Field(default_factory=list, description="Acknowledged unresolved risks")
    selected_guardrails: List[str] = Field(default_factory=list, description="Approved guardrail action IDs")
...
# Legacy / Frontend Compatibility Models

class Evidence(BaseModel):
    id: str = Field(..., description="Unique evidence ID")
    node_id: str = Field(..., description="Impact node ID this evidence supports")
    source: str = Field(..., description="Source system or artifact, e.g., Analytics / Mixpanel / Historical Cohorts")
    text: str = Field(..., description="Summary statement of empirical evidence")
    ...
    strength_score: float = Field(default=0.75, ge=0.0, le=1.0, description="Evidence strength score")
...
class Action(BaseModel):
    id: str = Field(..., description="Unique action ID")
    change_id: str = Field(..., description="Associated change ID")
    title: str = Field(..., description="Actionable recommendation title / action")
...
class Scenario(BaseModel):
    id: str = Field(..., description="Unique scenario ID")
    change_id: str = Field(..., description="Associated change ID")
    name: str = Field(..., description="Scenario title: 'CONSERVATIVE' | 'EXPECTED' | 'ADVERSE'")
    probability: float = Field(..., ge=0.0, le=1.0, description="Estimated probability of occurrence (must be coherent)")
    impact_description: str = Field(..., description="Summary of expected consequences in this scenario")
