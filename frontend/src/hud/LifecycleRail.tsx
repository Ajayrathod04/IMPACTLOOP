...
import { Eye, Shield, CheckCircle, Rocket, Activity, Brain } from 'lucide-react';
...
const STAGES: Array<{ id: LifecycleStage; label: string; icon: any; color: string; desc: string }> = [
  ...
  { id: 'DECIDE', label: 'DECIDE', icon: CheckCircle, color: '#FF6B5E', desc: 'Human Decision Gate & guardrails' },
...
export const LifecycleRail: React.FC<LifecycleRailProps> = ({
  ...
  return (
    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-40 max-w-4xl w-[92%] select-none">
      <div className="px-3 py-2 rounded-2xl bg-[#080B12]/90 border border-[#8B7CFF]/30 backdrop-blur-xl shadow-[0_10px_40px_rgba(0,0,0,0.8)] flex items-center justify-between gap-1 overflow-x-auto">
          ...
          return (
            <React.Fragment key={stage.id}>
              <button
                ...
                className={`flex-1 min-w-[110px] py-2 px-3 rounded-xl transition-all duration-300 flex flex-col items-center gap-1 group ${
                  isActive
                    ? 'bg-[#131826] border border-[#F4C95D] shadow-[0_0_20px_rgba(244,201,93,0.3)] scale-105'
                    : 'hover:bg-[#0E1320] border border-transparent'
                }`}
              >
