...
export const ScenarioComparisonView: React.FC<ScenarioComparisonViewProps> = ({ onSelectScenario }) => {
  ...
  const scenarios = [
    {
      ...
      estimated_impact: 'Minimal activation drop (-3%). Conversion velocity increases by 15%.',
      recommendation: 'Direct 100% rollout with automated trial extension fallback.',
      border: 'border-emerald-800/60 bg-emerald-950/20 hover:border-emerald-500/60',
    },
    {
      id: 'expected',
      ...
      estimated_impact: 'Temporary -8.0% activation drop balanced by +22% sales outreach velocity.',
      recommendation: 'Deploy 10% A/B cohort rollout with Day-3 email re-engagement.',
    ...
    },
    {
      ...
      estimated_impact: 'Severe -15% activation drop and 28% increase in Zendesk support volume.',
      recommendation: 'Implement instant 14-day trial extension button on Day 6 exit prompt.',
      ...
      <div className="bg-[#0b0e17] border border-slate-800/90 rounded-xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xl">
        <div>
          ...
          <p className="text-xs text-slate-400 font-mono mt-1">
            Compare conservative, expected baseline, and adverse stress outcomes before rollout approval.
          </p>
        </div>
      ...
      <div className="bg-[#0b0e17] border border-slate-800/90 rounded-xl p-4 space-y-2">
        ...
        <div className="grid grid-cols-3 gap-2 font-mono text-center text-xs">
          <div className="bg-emerald-950/40 p-2 rounded border border-emerald-800/40">
            <span className="text-[9px] text-slate-400 block uppercase">CONSERVATIVE</span>
            <span className="text-emerald-400 font-bold text-sm">-3% Activation</span>
          </div>
          <div className="bg-amber-950/40 p-2 rounded border border-amber-800/40">
            <span className="text-[9px] text-slate-400 block uppercase">EXPECTED BASELINE</span>
            <span className="text-amber-400 font-bold text-sm">-8% Activation</span>
          </div>
          <div className="bg-rose-950/40 p-2 rounded border border-rose-800/40">
            <span className="text-[9px] text-slate-400 block uppercase">ADVERSE STRESS</span>
            <span className="text-rose-400 font-bold text-sm">-15% Activation</span>
          </div>
        </div>
      </div>
          ...
          return (
            <div
              ...
              className={`border rounded-xl p-5 space-y-4 transition-all cursor-pointer relative ${sc.border} ${
                isSelected ? 'ring-2 ring-amber-400 shadow-2xl scale-[1.01]' : 'opacity-90 hover:opacity-100'
              }`}
            >
              ...
              <div>
                <h3 className="text-sm font-bold text-ivory-100 font-mono tracking-tight">{sc.title}</h3>
                <div className="flex items-center gap-3 mt-1 text-xs text-slate-400 font-mono">
                  <span>Risk Score: <strong className="text-ivory-100">{sc.risk_score}/100</strong></span>
                  <span>Confidence: <strong className="text-emerald-400">{sc.confidence}</strong></span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 bg-[#07090e] p-2.5 rounded border border-slate-800/90 text-[11px] font-mono text-center">
                <div>
                  <span className="text-[9px] text-slate-500 block uppercase">Activation</span>
                  <span className="font-bold text-amber-400">{sc.activation_delta}</span>
                </div>
                <div>
                  <span className="text-[9px] text-slate-500 block uppercase">Revenue</span>
                  <span className="font-bold text-emerald-400">{sc.revenue_impact.split(' ')[0]}</span>
                </div>
                <div>
                  <span className="text-[9px] text-slate-500 block uppercase">Support</span>
                  <span className="font-bold text-orange-400">{sc.support_load}</span>
                </div>
              </div>

              <div className="space-y-1 bg-[#07090e] p-3 rounded border border-slate-800/80 text-xs font-sans">
                <span className="text-[10px] font-mono uppercase text-slate-400 block font-semibold">Estimated Consequence</span>
                <p className="text-slate-200 leading-snug">{sc.estimated_impact}</p>
              </div>

              <div className="space-y-1 text-xs pt-2 border-t border-slate-800/80 font-mono">
                <span className="text-[10px] uppercase text-amber-400 block font-bold">Recommended Strategy</span>
                <p className="text-slate-300 text-[11px] leading-relaxed font-sans">{sc.recommendation}</p>
              </div>
