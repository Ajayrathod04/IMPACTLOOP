  ...
  const memoryItems = [
    {
      ...
      confidence: '94%',
      evidence_count: 3,
      key_finding: 'Trial length strongly affects activation latency. Reducing duration from 14 to 7 days drops signup conversion by 5.2% unless paired with Day-3 email drips.',
      reusable_rule: 'Trial duration changes historically affected activation more than top-of-funnel acquisition.',
    ...
    },
    {
      ...
      confidence: '98%',
      evidence_count: 5,
      key_finding: 'Billing dependency must be checked before pricing tier changes to prevent silent webhook failures.',
      reusable_rule: 'Billing dependency must be checked before pricing changes.',
    ...
    },
    {
      ...
      confidence: '91%',
      evidence_count: 4,
      key_finding: 'Lifecycle email sequence depends directly on trial-state event triggers.',
      reusable_rule: 'Activation impact appears before revenue impact in customer support telemetry.',
    ...
    },
    {
      ...
      confidence: '96%',
      evidence_count: 2,
      key_finding: 'Pricing experiment increased support volume by 18% during initial 14-day transition period.',
    ...
    },
    {
      ...
      confidence: '99%',
      evidence_count: 6,
      key_finding: 'Publishing HTTP Sunset headers 30 days prior prevents 75% of partner integration failures.',
      reusable_rule: 'Mandate 30-day HTTP Sunset header window for breaking REST API updates.',
      relevance: 'Critical Standard',
    },
  ];
    ...
    <div className="space-y-6 select-none">
      ...
      <div className="bg-[#0b0e17] border border-slate-800/90 rounded-xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xl">
        <div>
          ...
          <p className="text-xs text-slate-400 font-mono mt-1">
            Search organizational rules, historical outcomes, known dependencies, and validated guardrails.
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-[#0b0e17] p-3 rounded-xl border border-slate-800/90 font-mono text-xs">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto">
          {['all', 'VALIDATED ASSUMPTION', 'KNOWN DEPENDENCY', 'PAST OUTCOME', 'LESSONS LEARNED', 'GUARDRAIL'].map((type) => (
            <button
              key={type}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredItems.map((item) => (
          <div key={item.id} className="bg-[#0b0e17] border border-slate-800/90 rounded-xl p-5 space-y-3 hover:border-amber-500/50 transition-all shadow-xl">
            ...
            <h3 className="text-base font-bold text-ivory-100 font-sans">{item.title}</h3>
            ...
            <div className="space-y-2 text-xs">
              <div className="bg-[#07090e] p-3 rounded border border-slate-800/90 space-y-1">
                ...
                <p className="text-slate-300 leading-snug font-sans">{item.key_finding}</p>
              </div>
              ...
              <div className="bg-emerald-950/20 p-3 rounded border border-emerald-900/40 space-y-1">
                <span className="text-[10px] font-mono text-emerald-400 uppercase font-bold flex items-center gap-1">
                  <ShieldCheck className="w-3.5 h-3.5" /> Reusable Guardrail Rule
                </span>
                <p className="text-emerald-200 leading-snug font-sans font-medium">{item.reusable_rule}</p>
              </div>
            </div>
            ...
            <div className="pt-2 border-t border-slate-800/80 flex items-center justify-between text-[11px] text-slate-400 font-mono">
              <span className="flex items-center gap-1">
                ...
                Evidence Base: <strong className="text-slate-200">{item.evidence_count} Cohorts</strong>
              </span>
